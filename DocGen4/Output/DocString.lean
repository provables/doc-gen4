...
def preProcessMd (docString: String) : HtmlM String :=
  let l := docString.splitOn "\n" |>.map (fun line =>
    if line.startsWith "OEIS: " then
      let num := line.splitOn " " |>.getD 1 "0" |>.toNat? |>.getD 0
      ""
    else
      default
  )
